...
fn main(
    ...
    let flagByte = instanceFlagByte(instance);
    // Bit 7 exists only in the temporary offscreen capture buffer.
    if ((flagByte & 128u) != 0u) { return; }
    let state = flagByte & 3u;
    if ((uniforms.selectionPreview == 1u && (state & 1u) == 0u)
        || (uniforms.selectionPreview == 2u && (state & 1u) != 0u)) {
        return;
    }
