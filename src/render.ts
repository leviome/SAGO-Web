...
const registerRenderEvents = (scene: Scene, events: Events) => {
    ...
    events.function('render.offscreen', async (width: number, height: number): Promise<Uint8Array> => {
        const overlays = scene.camera.renderOverlays;
        const gizmos = scene.gizmoLayer.enabled;
        const centers = scene.centersLayer.enabled;
        const clearColor = scene.camera.camera.clearColor.clone();
        try {
            ...
            scene.camera.renderOverlays = overlays;
            scene.gizmoLayer.enabled = gizmos;
            scene.centersLayer.enabled = centers;
            scene.camera.camera.clearColor.copy(clearColor);
