...
class ProjectedSplatRenderer {
    // Temporary offscreen subset, independent of editable instance flags.
    captureFilter: { splat: Splat; flags: StorageBuffer } | null = null;
    // Viewport only: 0 = all, 1 = selected foreground, 2 = background.
    selectionPreview = 0;
    ...
    private getVariant(bands: number) {
        ...
        const uniformBufferFormat = new UniformBufferFormat(this.device, [
            ...
            new UniformFormat('selectionPreview', UNIFORMTYPE_UINT),
    ...
    render(forPick = false) {
        ...
        // Offscreen SAM/drone/export captures must always receive the complete scene.
        const selectionPreview = !camera.targetSizeOverride && !this.captureFilter && selectedSplat?.numSelected ? this.selectionPreview : 0;
        const occlusion = !selectionPreview && !this.captureFilter && this.stochastic && this.occlusionCull && this.prevValid &&
        ...
        for (const placement of this.placements) {
            ...
            compute.setParameter('instanceFlags', this.captureFilter?.splat === splat ? this.captureFilter.flags : instances.instanceFlags);
            ...
            compute.setParameter('visible', splat.visible && (!this.captureFilter || this.captureFilter.splat === splat) &&
                (selectionPreview !== 1 || selectedSplat === splat) ? 1 : 0);
            compute.setParameter('selectionEnabled', selectionEnabled ? 1 : 0);
            compute.setParameter('selectionPreview', selectedSplat === splat ? selectionPreview : 0);
